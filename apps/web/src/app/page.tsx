'use client'

import { useEffect, useState } from 'react'
import { Sparkles, Github } from 'lucide-react'
import { useAuth } from '@/hooks/use-auth'

export default function LoginPage() {
  const { user, loading, loginWithGithub, loginWithGoogle } = useAuth()
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    if (!loading && user) {
      // Use window.location for reliable navigation in static export
      window.location.href = '/todos'
    }
  }, [user, loading])

  const handleGithubLogin = async () => {
    setError(null)
    try {
      await loginWithGithub()
    } catch {
      setError('ログインに失敗しました。もう一度お試しください。')
    }
  }

  const handleGoogleLogin = async () => {
    setError(null)
    try {
      await loginWithGoogle()
    } catch {
      setError('ログインに失敗しました。もう一度お試しください。')
    }
  }

  if (loading) {
    return (
      <main className="flex min-h-screen items-center justify-center" data-testid="login-loading">
        <div className="animate-pulse text-zinc-400">Loading...</div>
      </main>
    )
  }

  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-8 px-4">
      <div className="flex flex-col items-center gap-4">
        <div className="flex items-center gap-2">
          <Sparkles className="h-8 w-8 text-[var(--color-primary)]" data-testid="logo-icon" />
          <span className="text-2xl font-bold tracking-tight">todo-with-any-ai</span>
        </div>
        <p className="text-center text-zinc-500 dark:text-zinc-400">
          AIエージェントのためのTodoアプリ。<br />人間も使える。
        </p>
      </div>

      <div className="flex w-full max-w-xs flex-col gap-3">
        <button
          onClick={handleGithubLogin}
          className="flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-zinc-900 text-white transition-opacity hover:opacity-90 dark:bg-zinc-800"
        >
          <Github className="h-5 w-5" />
          GitHubでログイン
        </button>

        <button
          onClick={handleGoogleLogin}
          className="flex h-12 w-full items-center justify-center gap-2 rounded-lg border border-zinc-300 bg-white text-zinc-700 transition-opacity hover:opacity-90 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-300"
        >
          <svg className="h-5 w-5" viewBox="0 0 24 24">
            <path
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
              fill="#4285F4"
            />
            <path
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              fill="#34A853"
            />
            <path
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              fill="#FBBC05"
            />
            <path
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              fill="#EA4335"
            />
          </svg>
          Googleでログイン
        </button>
      </div>

      {error && (
        <div role="alert" className="text-sm text-red-500">
          {error}
        </div>
      )}
    </main>
  )
}
